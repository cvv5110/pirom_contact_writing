...
Consider a DG-FEM as in Eq. [X] for $M$ elements, and a LCM as in Eq. [X] for $N$ components with $N_{\Gamma}$ interfaces; clearly, $M\gg N$. Let $\cV_j$, $\cF_i^-$, and $\cF_i^+$ be the sets of indices for the elements contained in component $j$, and the elements adjacent to interface $i$ on the negative and positive sides, respectively. Formally,
\begin{subequations}
    \begin{align}
    ...
    \end{align}
\end{subequations}
Therefore, the average temperature for component $j$ and the average temperatures on the negative and positive sides of interface $i$ are given by,
\begin{subequations}
    \begin{align}
    ...
    \end{align}
\end{subequations}
where $\left|\Omega_j\right|$ and  $\left|\Gamma_i\right|$ are the area $(d=2)$ or volume $(d=3)$ of component $j$, and the area of the interface $i$ between two different components, respectively. The orthogonal basis functions are defined as $\vvarphi_i^j=\left[1,0,\dots,0\right]\in\mathbb{R}^{P}$.
