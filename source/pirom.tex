...
Consider a DG-FEM as in Eq. [X] for $M$ elements, and a LCM as in Eq. [X] for $N$ components with $N_{\Gamma}$ interfaces; clearly, $M\gg N$. Let $\cV_j$, $\cF_i^-$, and $\cF_i^+$ be the sets of indices for the elements contained in component $j$, and the elements adjacent to interface $i$ on the negative and positive sides. 

An additional set of DG-FEM states, $\vw=\left[\vw_1^\top,\vw_2^\top,\dots,\vw_M^\top\right]^\top\in\mathbb{R}^{KP}$, is introduced to denote the subset of DG-FEM states that are adjacent to the imperfect contact interfaces, where $K$ is the number of elements adjacent to the imperfect contact interfaces. Clearly, $M>K$, since the number of elements is much larger than the number of elements adjacent to the imperfect contact interfaces.
...
\begin{subequations}
    \begin{align}
        ...
        \btheta_k^{-}(t) &= \frac{1}{\left|\Gamma_k\right|}\sum_{m\in\cF_k^-}\int_{\Gamma_k\cap\partial E_m} \vphi_m^\top\vw_m d\Gamma_k, \: k=1,\ldots,N_{\Gamma}\\
        \btheta_k^{+}(t) &= \frac{1}{\left|\Gamma_k\right|}\sum_{n\in\cF_k^+}\int_{\Gamma_k\cap\partial E_n} \vphi_n^\top\vw_n d\Gamma_k, \: k=1,\ldots,N_{\Gamma}
    \end{align}
\end{subequations}
where $\left|\Omega_j\right|$ and  $\left|\Gamma_k\right|$ are the area $(d=2)$ or volume $(d=3)$ of component $j$, and the area of the interface $k$ between two different components, respectively. The orthogonal basis functions are defined as $\vvarphi_i^j=\left[1,0,\dots,0\right]\in\mathbb{R}^{P}$.

Therefore, it follows that,
\begin{subequations}
    \begin{align}
        \bar{\theta}_j^{-}(t) &= \sum_{i\in\cF_j^{-}}\frac{\left|\Gamma_i\right|}{\left|\Gamma_j\right|}\bar{u}_i = \sum_{i\in\cF_j^{-}}\frac{\left|\Gamma_i\right|}{\left|\Gamma_j\right|}\vvarphi_i^{j\top}\vu_i(t)
    \end{align}
\end{subequations}
...
\begin{subequations}
    \begin{align}
        \delta\theta_i^{-}(\vu,t) &= T_i^{-}(\vu,t) - \btheta_i^{-}(\vu,t)\\
        \delta\theta_i^{+}(\vu,t) &= T_i^{+}(\vu,t) - \btheta_i^{+}(\vu,t)
    \end{align}
\end{subequations}
so that,
\begin{subequations}
    \begin{align}
        \Delta\theta_i(\vu,t) &= \left(\btheta_i^{-}(t) - \btheta_i^{+}(t)\right)  + \left(\delta\theta_i^{-}(\vu,t) - \delta\theta_i^{+}(\vu,t)\right)\\
        \btheta_i^{-}(t) &= C_i^{-}\vu(t)\\
        \btheta_i^{+}(t) &= C_i^{+}\vu(t)\\
        \Delta\vbtheta(t) &= \vPsi^+\vu(t)
    \end{align}
\end{subequations}
where,
\begin{equation}
...
\begin{subequations}
    \begin{align}
        \vu_i(t) &= \sum_{k=1}^{N}\vvarphi_k^{i\top}\bar{u}_k(t) + \delta\vu_i(t),\: i=1,\dots,N\\
        \Delta\theta_i(\vu,t) &= \sum_{k=1}^{N-1}\psi_k^i\Delta\btheta_k(t) + \delta\Delta\theta_i(\vu,t),\: i=1,\dots,N_{\Gamma}
    \end{align}
\end{subequations}
