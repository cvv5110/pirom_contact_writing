...
Define the coarse-grained states,
\begin{subequations}
    \begin{align}
        ...
        \btheta_i^{-}(t) &= \frac{1}{\left|\Gamma_i\right|}\sum_{m\in\cF_i^-}\int_{\Gamma_i\cap\partial E_m} \vphi_m^\top\vu_m d\Gamma_i, \: i=1,\ldots,N_{\Gamma}\\
        \btheta_i^{+}(t) &= \frac{1}{\left|\Gamma_i\right|}\sum_{n\in\cF_i^+}\int_{\Gamma_i\cap\partial E_n} \vphi_n^\top\vu_n d\Gamma_i, \: i=1,\ldots,N_{\Gamma}
    \end{align}
\end{subequations}
where $\left|\Omega_j\right|$ and $\left|\Gamma_i\right|$ are the area $(d=2)$ or volume $(d=3)$ of component $j$, and the area of the interface $i$ between two different components, respectively. The index sets $\cV_j$, $\cF_i^-$, and $\cF_i^+$ include the indices of elements that are contained in component $j$, and the indices of elements that are adjacent to the interface $\Gamma_i$ on the negative and positive sides, respectively. More formally, these index sets are defined as follows,
\begin{subequations}
    \begin{align}
...
Now, introducing the following constraint for the basis function,
\begin{equation}
    \int_{\Gamma_j\cap\partial E_m} \vphi_p^{j} d\Gamma = 0, p>2
\end{equation}
it follows that,
\begin{subequations}
    \begin{align}
        \bar{\theta}_i^{-}(t) &= \sum_{i\in\cF_j^{-}}\frac{\left|\Gamma_i\right|}{\left|\Gamma_j\right|}\vvarphi_i^{j\top}\vu_i(t)
    \end{align}
\end{subequations}
